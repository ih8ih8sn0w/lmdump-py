...
def defines(fp, fpw, symbol_list, atlas_list, positions_list, offset):
    ...
    num_0007s = integer(fp)
    num_sprites = integer(fp)
    num_000Bs = integer(fp)
    ...
    defines_temp.append(["\n\tNum unk_0007s: ", str(num_0007s)])
    defines_temp.append(["\n\tnum sprites: ", str(num_sprites)])
    defines_temp.append(["\n\tnum unk_000Bs: ", str(num_000Bs)])
    defines_temp.append(["\n\tnum texts: ", str(num_texts)])
    defines_temp.append(["\n\tunk2: 0x", format(unk2, "0>8X")])
    defines_temp.append(["\n\tunk3: 0x", format(unk3, "0>8X")])
    defines_temp.append(["\n\tunk4: 0x", format(unk4, "0>8X")])

    defines_list.append(defines_temp)
    for i in defines_temp:
        print(i)
    defines_temp = []
    ...
    for x in range(num_0007s):
        offset = fp.tell()
        chunk = fp.read(4)
        if chunk == bytes.fromhex('00000007'):
            defines_temp.append(unk_0007(fp, fpw, x, offset))
        else:
            print("Error: there should be a 0007 chunk here at 0x", str(format(offset, "0>8X")))
            exit()
    ...
    for x in range(num_000Bs):
        offset = fp.tell()
        chunk = fp.read(4)
        if chunk == bytes.fromhex('0000000B'):
            defines_temp.append(unk_000B(fp, fpw, x, offset))
...
def shape(fp, fpw, x, symbol_list, atlas_list, offset):
    ...
    if (dword_length == 4):
        ...
        shape_temp = ["\n\n\t\tSmall Shape # ", str(x), " offset: 0x", str(format(offset, "0>8X")), "\n\t\t{"]
        shape_temp.append(["\n\t\t\tCharacter ID: 0x", str(format(chr_id, "0>8X"))])
        shape_temp.append(["\n\t\t\tunk0: 0x", str(format(unk0, "0>8X"))])
        shape_temp.append(["\n\t\t\tunk1: 0x", str(format(unk1, "0>8X"))])
        shape_temp.append(["\n\t\t\tnum graphics?: 0x", str(format(num_graphics, "0>8X"))])
    else:
        shape_temp = ["\n\n\t\tShape # ", str(x), " offset: 0x", str(format(offset, "0>8X")), "\n\t\t{"]

        chr_id = integer(fp)
        unk0 = integer(fp) # fill style?
        bounds_id = integer(fp)
        num_quads = integer(fp)
        num_graphics = integer(fp)

        shape_temp.append(["\n\t\t\tCharacter ID: 0x", str(format(chr_id, "0>8X"))])
        shape_temp.append(["\n\t\t\tUnk 0: 0x", format(unk0, "0>8X")])
        shape_temp.append(["\n\t\t\tBounds ID: ", str(bounds_id)]) # oh
        shape_temp.append(["\n\t\t\tNum Quads: ", str(num_quads)])
    ...
    for z in range(num_graphics):
        ...
        elif chunk == bytes.fromhex('0000F023'):
            shape_list.append(unknowns(fp, fpw, z))
            # shape_list.append(quads(fp, fpw, z, atlas_list, x, offset));
        else:
...
def unk_0007(fp, fpw, x, offset):
    ...
    unk3 = integer(fp)
    unk4 = integer(fp)
    f018_count = integer(fp)
    unk5 = integer(fp)
    
    unk_0007_temp.append(["\n\t\tUnk 0007 # ", x, ": 0x", str(format(offset)), "\n\t\t{"])
    unk_0007_temp.append(["\n\t\t\tUnk 0: 0x", format(unk0, "0>8X")])
    unk_0007_temp.append(["\n\t\t\tUnk 1: 0x", format(unk1, "0>8X")])
    unk_0007_temp.append(["\n\t\t\tUnk 2: 0x", format(unk2, "0>8X")])
    unk_0007_temp.append(["\n\t\t\tUnk 3: 0x", format(unk3, "0>8X")])
    unk_0007_temp.append(["\n\t\t\tUnk 4: 0x", format(unk4, "0>8X")])
    unk_0007_temp.append(["\n\t\t\tNum f018s: ", str(f018_count)])
    unk_0007_temp.append(["\n\t\t\tUnk 5: 0x", format(unk5, "0>8X")])
    unk_0007_temp.append(["\n\t\t\t{"])
    for i in range(f018_count):
        chunk = fp.read(4)
        if chunk == bytes.fromhex('0000f018'):
            unk_0007_temp.append(unk_f018(fp, fpw, i))
        else:
            print("Error, expected F018 at 0x", str(format(fp.tell() - 4)), "\nIn unk_0007: 0x", str(format(offset, "0>8X")))
            exit()

    unk_0007_temp.append(["\n\t\t\t}"])

def unk_f018(fp, fpw, x):
    f018_temp = []
    offset = fp.tell() - 8
    dword_length = integer(fp)
    ...
    f018_temp.append(["\n\t\t\t\tUnk f018 # ", str(x), " offset: 0x", str(format(offset, "0>8X")), "\n\t\t\t\t{"])
    f018_temp.append(["\n\t\t\t\t\tUnk 0: 0x", format(unk0, "0>8X")])
    f018_temp.append(["\n\t\t\t\t\tUnk 1: 0x", format(unk1, "0>8X")])
    f018_temp.append(["\n\t\t\t\t\tUnk 2: 0x", format(unk2, "0>8X")])
    f018_temp.append(["\n\t\t\t\t\tUnk 3: 0x", format(unk3, "0>8X")])
...
def unk_000B(fp, fpw, x, offset):
    ...
    unk_000B_temp.append(["\n\t\tUnk 000B # ", str(x), " offset: 0x", str(format(offset, "0>8X")), "\n\t\t{"])
    unk_000B_temp.append(["\n\t\t\tUnk 0: 0x", format(unk0, "0>8X")])
    unk_000B_temp.append(["\n\t\t\tUnk 1: 0x", format(unk1, "0>8X")])
    unk_000B_temp.append(["\n\t\t\tUnk 2: 0x", format(unk2, "0>8X")])
    unk_000B_temp.append(["\n\t\t\tUnk 3: 0x", format(unk3, "0>8X")])
    unk_000B_temp.append(["\n\t\t\tNum F00Es: ", str(num_F00E)])
    
    unk_000B_temp.append(["\n\t\t\tF00Es:"])
    unk_000B_temp.append(["\n\t\t\t{"])
    for x in range(num_F00E):
        offset = fp.tell()
        chunk = fp.read(4)
        if chunk == bytes.fromhex('0000F00E'):
            unk_000B_temp.append(unk_F00E(fp, fpw, x, offset))
        else:
            print("Expected a F00E chunk here at: 0x", str(format(offset, "0>8X")))
            exit()
    unk_000B_temp.append(["\n\t\t\t}"])
    unk_000B_temp.append(["\n\t\t}"])
    
    return unk_000B_temp
    
def unk_F00E(fp, fpw, x, offset):
    unk_F00E_temp = []
    ...
    unk_F00E_temp.append(["\n\t\t\t\tUnk F00E # ", str(x), " offset: 0x", str(format(offset, "0>8X")), "\n\t\t\t\t{"])
    unk_F00E_temp.append(["\n\t\t\t\t\tUnk 0: 0x", format(unk0, "0>8X")])
    unk_F00E_temp.append(["\n\t\t\t\t\tUnk 1: 0x", format(unk1, "0>8X")])
    unk_F00E_temp.append(["\n\t\t\t\t\tUnk 2: 0x", format(unk2, "0>8X")])
    unk_F00E_temp.append(["\n\t\t\t\t\tUnk 3: 0x", format(unk3, "0>8X")])
